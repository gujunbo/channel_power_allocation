...
\While{Operation profile is different from previous round}{
	\For{$i\in \mathcal{N}$}{
			\If{$i$ is idle}{
			$i$ choose channel $c$, where \\
			$\sum f^c_t \leq THOLD^c_t, \forall t\in \mathcal{T}$, and $ \gamma^c_i > \gamma_i^{c'}$, $c'\in \mathbb{C}\setminus c$
			}
			\If{$i$ is operating}{
						\If{$ f^{c'}_i < f_i^{c(i)}$ \textbf{and} $\sum f^{c'}_t \leq THOLD^{c'}_t$}{
						$c(i)= c'$					
						}
			}
	}
