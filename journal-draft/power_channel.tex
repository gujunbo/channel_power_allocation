...
\label{QLP_2}
			\begin{aligned}
			\underset{}{\text{minimize}}
			& \sum\limits^{n}_{i=1} ( \sum\limits_{j\in\mathcal{N}, j\neq i}\sum\limits_{k=1}^{|\mathcal{C}|} \frac{p_j\cdot h_{ji}\cdot z_{ji}}{p_i\cdot h_{ii}\cdot z_{ii}}\cdot  x_j^k\cdot x_i^k  \\
			& + \sum\limits_{k=1}^{|\mathcal{C}|} \frac{N_0}{p_i\cdot h_{ii}\cdot z_{ii}}\cdot x_i^k)\\
			\text{subject to} 
			& \sum\limits_{k=1}^{|\mathcal{C}|}x_i^k=1, x_i^k\in X_i\in \{0,1\}^{|\mathcal{C}|}\\
...
Now the objective function is is quadratic as shown in Formula~\ref{scheme2-obj-intermediate},
	...
	\sum\limits^{n}_{i=1} (h_{ji}z_{ji} \sum\limits_{j\in\mathcal{N}, j\neq i}  \sum\limits_{k=1}^{|\mathcal{C}|}  \beta_{ij}^k q_i  + N_0\sum\limits_{k=1}^{|\mathcal{C}|}  x_i^k q_i )
	\end{equation}
	
	but the constraint \ref{q_opt} is a quadratic equality, which makes the problem to be non-convex. 
	...
	\For{$i\in \mathcal{N}$}{
		\For{$c\in \mathcal{C}\setminus c(i)$}{
		 \If{$\sum f^c_m \leq Threshold^c$}{
			\If{$c(i)\in \mathcal{C} \And{}  f^c_i < f_i$}{
			$f_i = f^c_i$\\
			$c(i) = c$
		 }
		}
