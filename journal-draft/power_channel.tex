...
These unlicensed devices are called white space devices (WSDs) and their operation should be restricted in terms of location, channel and transmission power, so that no harmful interference is disturbing the incumbents.
...
WSDB plays the key role in the utilization of TVWS, which has the global view of the WSDs in the network and protects the DTT receivers.
The resource which is available for the WSDs is decided by the strategy of WSDB, which is as per the regulators.
Ofcom and ECC impose restrictions on the aggregated interference caused by TVDBs on the DTT receivers, and implicitly allow TVDBs to have different transmission powers based on their locations.
...
In this paper, we will follow the most prevalent regulations on the usage of TVWS and investigate the spectrum sharing issue among the WSDs.
...
The WSDs are classified into two functional categories: 1) High power, stationary stations such a base stations , 2) lower power personal/portable devices, such as Wi-Fi network interface cards in laptops etc..
...
As the fixed WSDs work with high transmission power, they are the major potential source of interference to the DTT receivers so that their transmission characteristics should be carefully decided by the TWDB.
...
In this paper, we investigate efficient schemes to exploit the TV spectrum among the fixed WSDs which work as base station in IEEE 802.22 network.
...
We consider an IEEE 802.22 compliant cellular network where the fixed WSDs work as base station and provide broadband access to their terminals.
The network is illustrated in Figure~\ref{sysmodel}.
We call the fixed WSDs which work as base stations as White space Base Stations (WBSs), they are located in one area which is surrounded by DTV stations and receivers.
Several critical points are deployed in the vicinity of the DTV receivers which are the most venerable to the interference caused by the WSDs.
WBSs work in underlay manner and coexist with the DTV stations and receivers, the aggregate interference generated by WBSs should not exceed the threshold on each channel at each critical point.
...
All the WBSs work with the channels approved by the TVDB, they operate with a channel from the approved ones after choosing it, thus we omit the time index in the channel usage.
...
According to regulations, the calculation engine within the TVDB translates the information of incumbent services and technical characteristics and location of the WSDs into a list of allowed frequencies and associated transmit powers for the WSDs~\cite{ECC236}.
TVDB's decisions on the transmission power are different with respect to different regulations.
...
%As to the decision made on the transmission power per FCC, all WBSs work with 4 Watts transmission power.
...
After executing Algorithm \ref{whitecatalgo}, WBSs report to TVDB about their chosen channels if they are updated.
...
\item \textit{Potential Game}: A potential game based distributed scheme which is introduced in~\cite{spectrum_sharing_tvspace_2012}.
