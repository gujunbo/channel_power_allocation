...
We call the fixed white space devices which work as base stations as White space Base Stations (WBSs), they are located in one area which is surrounded by DTV stations and receivers.
Several critical points are deployed in the vicinity of the DTV receivers which are the most venerable to the interference caused by the white space devices.
...
\section{Conclusions}
In this paper, we look into the problem that how to exploit the TV white space in a IEEE 802.22 like cellular network.
Both centralized and distributed solutions for channel allocation problem, with respect to both ECC and FCC regulations, are proposed.
