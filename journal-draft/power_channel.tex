...
\label{QLP_2}
			\begin{aligned}
			\underset{}{\text{minimize}}
			& \sum\limits^{n}_{i=1} ( \sum\limits_{j\in\mathcal{N}, j\neq i}\sum\limits_{k=1}^{|\mathcal{C}|} \frac{p_j\cdot h_{ji}\cdot z_{ji}}{p_i\cdot h_i\cdot z_i}\cdot  x_j^k\cdot x_i^k  \\
...
%& p_i \cdot q_i   \geq 	q_i P_\mu + p_i P_\mu - P_\mu^2\\
%& p_i \cdot q_i   \geq 	q_i P_{\mathtt{op}} + p_i P_{\mathtt{op}} - P_{\mathtt{op}}^2\\
   \end{aligned}
	\end{equation}
...
%	& P_\mu(P_\mu - P_{\mathtt{op}}) (y_i + z_i) + 2P_{\mathtt{op}}P_\mu -P_\mu^2 \leq 1\\
%	& P_{\mathtt{op}} (P_\mu - P_{\mathtt{op}}) (y_i + z_i) + P_{\mathtt{op}}^2 \leq 1\\
   \end{aligned}
	\end{equation}	
...
%			& \sum\limits^{n}_{i=1}x_i^k p_i h_{i,pt} z \leq I^k_{pt}\\
