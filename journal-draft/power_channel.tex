...
The second scheme is similar with the FCC proposals in terms of transmission power, \ie WBSs operate with the same transmission power $P_{\mathtt{op}}$.
In this scheme, when all or many WBSs operate on the same channel, the sum of interference may cause interference to TV receivers, so that the number of WBSs operating on a certain channel should be restricted.
In other words, not all WBSs can operate and the problem is how to find out those WBSs and let them be idle.
...
\label{QLP}
		...
		& \text{subject to}
		&  \sum\limits_{k=1}^{|\mathcal{C}|}x_i^k=1, x_i^k\in X_i\in \{0,1\}^{|\mathcal{C}|}\\
		& & p_i^k\in \{P_\mu, P_{\mathtt{op}}\}, \mathtt{for} \forall i\in \mathcal{N}, \forall k\in \mathcal{C}\\
...
\label{QLP_2}
			...
			\underset{}{\text{minimize}}
			& \sum\limits^{n}_{i=1} ( \sum\limits_{j\in\mathcal{N}, j\neq i}\sum\limits_{k\in\mathcal{C}} \frac{p_j^k\cdot h_{ji}\cdot z_{ji}}{p_i^k\cdot h_i\cdot z_i}\cdot  x_j^k\cdot x_i^k  \\
			& + \sum\limits_k \frac{N_0}{p_i^k\cdot h_i\cdot z_i}\cdot x_i^k)\\
			\text{subject to} 
			& \sum\limits_{k=1}^{|\mathcal{C}|}x_i^k=1, x_i^k\in X_i\in \{0,1\}^{|\mathcal{C}|}\\
			& p_i^k\in \{P_\mu, P_{\mathtt{op}}\} \mathtt{for} \forall i\in \mathcal{N}, \forall k\in \mathcal{C}\\
			& \sum\limits^{n}_{i=1}x_i^k\cdot p_i^k\cdot h_{i,pt}\cdot z \leq I^k_{pt}, \mathtt{for}\forall k\in \mathcal{C} \\
			\end{aligned}
		\end{equation}
		
Formulation \ref{QLP_2} is nonlinear but we can transform it into a quadratic optimization with auxiliary variables.
We introduce binary number $\alpha$, real number $\beta$ and $q$, where
	\begin{equation}
	\label{alpha_opt}
...
\frac{1}{p_i^k} = q_i^k
	\end{equation}
Then the optimization~\ref{QLP_2} is transformed as:
	\begin{equation}
\label{scheme2optimizaiton}
			\begin{aligned}
			& \underset{}{\text{minimize}}\\
			&\sum\limits^{n}_{i=1}(\sum\limits_{j\in\mathcal{N}, j\neq i}\sum\limits_k q_{i}^k\cdot \beta_{ij}^k \cdot h_{ji}\cdot z + \sum\limits_k N_0\cdot q_{i}^k\cdot x_i^k)\\
			& \text{subject to:} \\
			& x_j^k + x_i^k - \alpha_{ij}^k\leq 1 \\
			& -x_j^k - x_i^k +2\cdot \alpha_{ij}^k \leq 0 \\
			& \beta_{ij}^k - p_{\mathtt{op}}\cdot \alpha_{ij}^k \leq 0 \\
			& - \beta_{ij}^k + p_\mu\cdot \alpha_{ij}^k \leq 0 \\
			& \beta_{ij}^k - p_j^k - p_\mu\cdot \alpha_{ij}^k \leq -p_j^{k,min} \\			
			& -\beta_{ij}^k + p_j^k + p_{\mathtt{op}}\cdot \alpha_{ij}^k \leq p_j^{k,max} \\			
			& \sum\limits_{k=1}^{|\mathcal{C}|}x_i^k=1, x_i^k\in X_i\in \{0,1\}^{|\mathcal{C}|}\\
			& q_i^k\cdot p_i^k =1\\
			& \sum\limits^{n}_{i=1}x_i^1\cdot p_i^1\cdot h_{i,pt}\cdot z \leq I^1_{pt}\\
			& \vdots\\
			& \sum\limits^{n}_{i=1}x_i^k\cdot p_i^k\cdot h_{i,pt}\cdot z \leq I^k_{pt}\\
			& \vdots\\
			&  \sum\limits^{n}_{i=1}x_i^{|\mathcal{C}|}\cdot p_i^{|\mathcal{C}|}\cdot h_{i,pt}\cdot z \leq I^{|\mathcal{C}|}_{pt}\\
			\end{aligned}
		\end{equation}
%		\todo{result is not ready.}

The objective function is quadratic and there is a quadratic equality constraint.
...
\DontPrintSemicolon
\SetAlgoLined
\KwIn{The complete graph of the network, \ie the distance, path lose and shadowing parameter between each WBS and any other WBS and every TV interference measurement devices ;\\ transmission power;\\Secondary network operation profile $c(i), f_i$ where $i\in \mathcal{N}$. $c(i)$ is the channel used by WBS $i$, $f_i$ is the co-channel interference received on WBS $i$.}

\While{Operation profile is different from previous round}{
	\For{$i\in \mathcal{N}$}{
		\For{$c\in \mathcal{C}\setminus c(i)$}{
		 \If{$\sum f^c_m \leq Threshold^c$}{
			\If{$c(i)\in \mathcal{C} \&\& f^c_i < f_i$}{
			$f_i = f^c_i$\\
			$c(i) = c$
		 }
		}
