...
\maketitle
\graphicspath{
{../figures/03_distributedChannelAllocation/}{../figure/03_distributedChannelAllocation/1mar2016/}{../figure/03_distributedChannelAllocation/1mar2016/radius_6000_runtimes_50/}
}
...
We will look in to a cellular network which complies with IEEE 802.22, where the base stations are the fixed WSDs which provide broadband access to their terminals.
...
The transmission power of WBS $i$ on channel $c$ is $P_i^c$, $c(i)$ denotes the channel used by a WBS $i\in \mathcal{N}$. 
...
As to a terminal $m$ which is associated to WBS $i$, the attenuation between WBS $i$ and itself is denoted as $h_{im}$, and the attenuation between a co-channel interfering WBS $j$ and $m$ is denoted as $h_{jm}$.
...
With an auxiliary circle centered at the discussed WBS, which is shown as dashed circle in Figure~\ref{quasiSINRfigure}, QuasiSINR is the ratio between the power of signal of interest on the circle and the summation of the strongest power from the interfering WBSs on the circle.
In~\ref{quasiSINRfigure}, assuming the radius of the auxiliary circle is $\delta$ and all the WBSs work on channel $c$, then QuasiSINR is a quotient where the divided is the WBS $i$'s power on the green pot, and the divisor is the summation of the power on the red pot, which is caused by the corresponding interfering WBS.
...
We summarize some notations here.
\begin{table}[h]
\caption{Notations}
\label{tab1}
\centering
...
In our model, WBSs access the TVDB and obtain the transmission parameters, \ie working channel, transmission power of the other WBSs and the attenuation characteristics between itself and those WBSs. Then every WBSs calculates its QuasiSINR.
...
\subsubsection*{Permitted Transmission Power for WBSs}
%According to regulations, WBSs rely on database queries to assign resources, \ie frequencies and transmission powers.
According to regulations, the calculation engine within the TVDB translates the information on incumbent services and the technical characteristics and location of the WSDs into a list of allowed frequencies and associated transmit powers for the WSDs.~\ref{ECC236}.
ECC and Ofcom propose guidance~\ref{ECC186}\ref{ECMA392} and focus on the protection of the critical points from the harmful interference, and don't regulate the power limitation. 
...
To address this fairness issue, we maximize the sum of the logarithmic value of every WBS's transmission power, and formulate the problem into a convex optimization problem.
	\begin{equation}
		...
		\end{aligned}
	\end{equation}
This optimization is conducted for each channel $c\in \mathbb{C}$.

As to decision on the transmission power per ECC, all WBSs work with 4 W transmission power.
Obviously there is a limit on the number of WBSs which operation.

\subsubsection*{Problem Formulation}
Our goal is to minimize the sum of inverted quasiSINR $\sum_{i\in \mathcal{N}}1/{\gamma_{i}}$ with the restrictions on the transmission powers of the WBSs.
We minimize the sum of inverted quasiSINR instead of maximizing the sum of quasiSINR of all WBSs, is to ensure the fairness among WBSs,  
%	\begin{equation}
%\label{problem}
%			\begin{aligned}
%			& {\text{Minimize}}
...
To address this fairness issue, we maximize the sum of the logarithmic value of every WBS's transmission power, and formulate the problem into a convex optimization problem.
	\begin{equation}
		...
		& & \sum_{i\in \mathcal{N}} \log P^c_i \\
		& \text{subject to}
		& & \sum_{i\in \mathcal{N}} (P^c_i \cdot h_{i\rightarrow \text{critical point}}\cdot z) < I^k_{pt}, 
		\end{aligned}
	\end{equation}
...
%Performance evaluation consists of two parts, in the first part whiteCat are compared with other distributed schemes proposed for the problem of channel allocation with different fixed transmission power, where the transmission power levels associated with WBSs and channels are different.
